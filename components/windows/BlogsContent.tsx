        ...
        <div className="flex-1 p-2 @sm:p-3 @md:p-4 @lg:p-6 @xl:p-8 overflow-auto text-white bg-[#1E1E1E] h-full @container">
            ...
            <div className="space-y-3 @sm:space-y-4 @md:space-y-5 @lg:space-y-6">
                {blogs.map((blog) => {
                    ...
                    return (
                    <div key={blog.id} className="flex flex-col @2xl:flex-row bg-[#2C2C2C] rounded-xl overflow-hidden border border-[#3E3E3E] group hover:border-blue-400/50 transition-colors hover:shadow-lg hover:shadow-blue-500/10">
                        {blog.cover_image_path && !hasError ? (
                            <div className="w-full @2xl:w-64 h-40 @sm:h-48 @2xl:h-auto relative bg-gray-800 shrink-0">
                                <Image
                            ...
                            <div className="w-full @2xl:w-64 h-40 @sm:h-48 @2xl:h-auto bg-gradient-to-br from-blue-900/40 to-purple-900/40 shrink-0 flex items-center justify-center border-b @2xl:border-r @2xl:border-b-0 border-[#3E3E3E]">
                                <BookOpen className="w-12 h-12 text-blue-400/50" />
                            </div>
                        )}

                        <div className="flex flex-col flex-1 p-3 @sm:p-4 @md:p-5 @lg:p-6 justify-center">
                            <h3 className="font-bold text-sm @sm:text-base @md:text-lg @lg:text-xl text-gray-100 mb-2 @sm:mb-3 group-hover:text-blue-400 transition-colors line-clamp-2">
                                {blog.title}
                            </h3>

                            <div className="flex flex-wrap items-center gap-2 @sm:gap-3 @md:gap-4 text-[9px] @sm:text-[10px] @md:text-xs font-medium text-gray-400 mb-2 @sm:mb-3 @md:mb-4">
                                {blog.published_at && (
                                    <div className="flex items-center gap-1 @sm:gap-1.5">
                                        <Calendar className="w-3 h-3 @sm:w-4 @sm:h-4 flex-shrink-0" />
                                        <span className="text-[8px] @sm:text-[9px] @md:text-xs">{formatDate(blog.published_at)}</span>
                                    </div>
                                )}
                                {blog.reading_time && (
                                    <div className="flex items-center gap-1 @sm:gap-1.5">
                                        <Clock className="w-3 h-3 @sm:w-4 @sm:h-4 flex-shrink-0" />
                                        <span className="text-[8px] @sm:text-[9px] @md:text-xs">{formatReadingTime(blog.reading_time)}</span>
                                    </div>
                                )}
                            </div>

                            {blog.excerpt && (
                                <p className="text-gray-300 text-[10px] @sm:text-xs @md:text-sm leading-relaxed mb-2 @sm:mb-3 @md:mb-4 line-clamp-3">
                                    {blog.excerpt}
                                </p>
                            )}
                            ...
                            {blog.url && (
                                <a
                                  ...
                                  className="mt-auto inline-flex items-center gap-1.5 @sm:gap-2 text-[9px] @sm:text-xs @md:text-sm font-bold text-blue-400 hover:text-blue-300 transition-colors w-fit"
                                >
                                    Read Article <ExternalLink className="w-3 h-3 @sm:w-4 @sm:h-4 flex-shrink-0" />
                                </a>
                            )}
                        </div>
                    </div>
